...
from lib.train_gaussian import loop
...
datamodes = ['train','validate']
def import_data(dset, mode):
    data = dset.load(mode)
    return data
dsets = Parallel(n_jobs=num_cores, max_nbytes=None)(delayed(import_data)(dset, mode) for mode in datamodes)
training_set = dsets[0]
validation_set = dsets[1]
