...
while table[table['in_progress'] == -1].isnull()['test_score'].sum() > 0:
    ...
    table.loc[idx, 'test_smape_mean'] = df_test[df_test.Quantile == 0.5]['sMAPE'].values
    table.loc[idx, 'test_nrmse_mean'] = df_test[df_test.Quantile == 0.5]['NRMSE'].values
    table.loc[idx, 'test_q50_mean'] = df_test[df_test.Quantile == 0.5]['QuantileLoss'].values
    table.loc[idx, 'test_mean_quantile_loss_mean'] = df_test['QuantileLoss'].mean()
    table.loc[idx, 'parameters'] = count_parameters(model)
