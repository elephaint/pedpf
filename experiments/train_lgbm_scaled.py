...
# Scale data
scaleY_train = 1 + X_train[:, :dim_inputseqlen, -1].mean(dim = 1)
scaleY_val = 1 + X_val[:, :dim_inputseqlen, -1].mean(dim = 1)
scaleY_test = 1 + X_test[:, :dim_inputseqlen, -1].mean(dim = 1)
X_train[:, :, -1] /= scaleY_train.unsqueeze(-1)
X_val[:, :, -1] /= scaleY_val.unsqueeze(-1)
X_test[:, :, -1] /= scaleY_test.unsqueeze(-1)
# Create numpy datasets in correct tabular format
train_data = create_data(X_train, y_train / scaleY_train, 0, dim_inputseqlen, dim_outputseqlen)
valid_data = create_data(X_val, y_val / scaleY_val, 0, dim_inputseqlen, dim_outputseqlen)
...
try:
    ...
    for key, value in study.best_trial.params.items():
        ...
        params['reg_alpha'] = 0.0005
        params['reg_lambda'] = 1
        params['num_leaves'] = 127
        params['feature_fraction'] = 0.75
        params['bagging_fraction'] = 0.50
...
# Apply scaling
yhat_tot_validate = (yhat_tot_validate * scaleY_val).numpy()
yhat_tot_test = (yhat_tot_test * scaleY_test).numpy()
# Compute statistics for all quantiles and entire prediction horizon
df_validate = calc_metrics(yhat_tot_validate, y_val.numpy(), quantiles)
df_test = calc_metrics(yhat_tot_test, y_test.numpy(), quantiles)
#%% Save
