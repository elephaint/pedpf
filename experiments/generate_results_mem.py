...
df = df[['GPU Clock [MHz]','Memory Clock [MHz]', 'GPU Temperature [°C]', 'GPU Load [%]' , 
         'Board Power Draw [W]','Power Consumption (%) [% TDP]','Memory Used [MB]']]
# Some processing
df['Memory Clock [MHz]'] = pd.to_numeric(df['Memory Clock [MHz]'], errors='coerce')
df['Power Consumption (%) [% TDP]'] = pd.to_numeric(df['Power Consumption (%) [% TDP]'], errors='coerce')
df['Memory Used [MB]'] = pd.to_numeric(df['Memory Used [MB]'], errors='coerce')
...
for dataset in datasets:
    ...
    for i in range(len(df_dataset)):
        ...
        memory_cons = df_current['Memory Used [MB]'].mean()
        # Save to file
        ...
        df_dataset.loc[i, 'memory'] = memory_cons
