...
file_experiments = experiment_dir + f'/experiments_memory_{dataset_name}.csv'
table = read_table(file_experiments)
d_emb = get_emb(dataset_name)
t_epochs = np.zeros((epochs))
while table[table['in_progress'] == -1].isnull()['end_time'].sum() > 0:
    # Read experiment table, set hyperparameters
    idx = table[table['in_progress'] == -1].isnull()['end_time'].idxmax()
